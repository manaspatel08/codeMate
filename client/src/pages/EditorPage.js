...
import ACTIONS from "../Actions"; // ✅ Correct path

const EditorPage = () => {
    const editorRef = useRef(null);
    const socketRef = useRef(null);
    const codeRef = useRef("");  // ✅ Initialize codeRef to prevent undefined errors
    const location = useLocation();
    const { roomId } = useParams();
    const reactNavigator = useNavigate();
    const [clients, setClients] = useState([]);
    ...
    return (
        <div className="min-h-screen flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/4 bg-gray-100 p-4 border-b md:border-r border-gray-300 flex flex-col">
                <div className="flex items-center justify-center">
                    <h3 className="text-lg font-semibold mb-2 text-gray-700">Connected Members</h3>
                </div>
                ...
                <button className="w-full bg-blue-500 text-white py-2 px-3 rounded hover:bg-blue-600 transition duration-300 mt-1" onClick={copyRoomId}>
                    Copy ROOM ID
                </button>
                <button className="w-full bg-red-500 text-white py-2 px-3 rounded hover:bg-red-600 transition duration-300 mt-1" onClick={leaveRoom}>
                    Leave Room
                </button>
            </div>
            ...
            <div className="w-full md:w-3/4 p-6 flex flex-col">
                ...
                <div className="flex-1 flex flex-col">
                    {/* Code Editor Section */}
                    <div className="flex-1 border-t border-gray-300 p-2 mb-4">
                        <Editor socketRef={socketRef} roomId={roomId} selectedLanguage={selectedLanguage} onCodeChange={handleCodeChange} />
                    </div>
                    {/* Output Section at the Bottom */}
                    <div>
                        <Output editorRef={codeRef} language={selectedLanguage} />
